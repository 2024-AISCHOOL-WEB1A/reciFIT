    ...
    return (
        <div className="mypage">
            ...
            <section ref={recipesRef} className="my-recipes">
                <button className="shared-title1" onClick={() => handleSectionClick('recipes')}>나의 레시피 조회</button>
                {recipes.length > 0 ? (
                    recipes.map(recipe => (
                ...
                {ingredients.length > 0 ? (
                    ingredients.map(ingredient => (
                        <div key={ingredient.u_ingre_idx}>
                            ...
                            <p>식재료 유통기한: {new Date(ingredient.expired_date).toLocaleDateString() || '없음'}</p>
                        </div>
                    ))
                ) : (
                    <p>식재료 정보가 없습니다.</p>
            ...
            <section ref={receiptsRef} className="my-receipts">
                <button className="shared-title1" onClick={() => handleSectionClick('receipts')}>영수증 관리</button>
                {receipts.length > 0 ? (
